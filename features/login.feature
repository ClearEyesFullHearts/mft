...
Test the server connection API
  ...
  Scenario: Known user can login
    ...
    And response body path $.user.username should be mft-user-test
  ...
  Scenario: Known admin can login
    ...
    And response body path $.user.username should be mft-admin-test
  ...
  Scenario: Cannot register a user with a known email
    Given I set body to {"username":"mft-test-2", "email":"mft-user-test@mathieufont.com", "password":"doubleuserpass"}
    When I POST to /users
